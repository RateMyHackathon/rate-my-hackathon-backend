...
router.get('/', (req, res) => {
  Hackathon.find()
    ...
    .catch(err => res.status(404).json({ nohackathonsfound: 'No Hackathons found' }));
});
...
router.get('/:id', (req, res) => {
  Hackathon.findById(req.params.id)
    ...
    .catch(err => res.status(404).json({ nohackathonfound: 'No Hackathon found' }));
});
...
router.post('/', (req, res) => {
  Hackathon.create(req.body)
    ...
    .catch(err => res.status(400).json({ error: 'Unable to add this hackathon' }));
});
...
router.delete('/:id', (req, res) => {
  Hackathon.findByIdAndRemove(req.params.id, req.body)
    .then(hackathon => res.json({ mgs: 'Hackathon entry deleted successfully' }))
